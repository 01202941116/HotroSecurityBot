...
# Optional keepalive web server (for Render Web Service)
def start_keepalive():
    try:
        from keepalive import run as run_server
        import threading
...
async def setflood(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not s:
        s = Setting(chat_id=update.effective_chat.id, flood_limit=n); db.add(s)
    else:
        s.flood_limit = n
    db.commit()
...
async def guard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ts.append(now)
    FLOOD[key] = ts
    if len(ts) > s.flood_limit and s.flood_mode == "mute":
        try:
            await context.bot.restrict_chat_member(chat_id, msg.from_user.id, ChatPermissions(can_send_messages=False), until_date=datetime.now()+timedelta(minutes=5))
        except Exception:
            pass
