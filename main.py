\
    ...
    def gen_key(months=1):
        ...
        conn = sqlite3.connect(DB); cur = conn.cursor()
        cur.execute("INSERT INTO pro_keys(key, months, created_at, expires_at) VALUES(?,?,?,?)", (key, months, created.isoformat(), expires.isoformat()))
        conn.commit(); conn.close()
        return key, expires
    ...
    def start(update: Update, context: CallbackContext):
        update.message.reply_text("HotroSecurityBot - moderation starter. Use /status to see config. Admins can use /genkey, /whitelist_add, /blacklist_add, etc.")
    ...
    def status(update: Update, context: CallbackContext):
        ...
        text += f\"nolinks={s['nolinks']}, noforwards={s['noforwards']}, nobots={s['nobots']}, antiflood={s['antiflood']}\\n\"
        text += f\"Whitelist: {', '.join(wl) if wl else '(none)'}\\nBlacklist: {', '.join(bl) if bl else '(none)'}\"
    ...
    def whitelist_add(update: Update, context: CallbackContext):
        ...
        add_whitelist(chat.id, txt)
        update.message.reply_text(f\"Added to whitelist: {txt}\")
    ...
    def whitelist_remove(update: Update, context: CallbackContext):
        ...
        txt = ' '.join(context.args).strip()
        remove_whitelist(chat.id, txt)
        update.message.reply_text(f\"Removed from whitelist: {txt}\")
    ...
    def blacklist_add(update: Update, context: CallbackContext):
        ...
        txt = ' '.join(context.args).strip()
        add_blacklist(chat.id, txt)
        update.message.reply_text(f\"Added to blacklist: {txt}\")
    ...
    def blacklist_remove(update: Update, context: CallbackContext):
        ...
        txt = ' '.join(context.args).strip()
        remove_blacklist(chat.id, txt)
        update.message.reply_text(f\"Removed from blacklist: {txt}\")
    ...
    def genkey_cmd(update: Update, context: CallbackContext):
        ...
        if context.args:
            try:
                ...
                update.message.reply_text(\"Usage: /genkey <months> (integer)\") ; return
        key, expires = gen_key(months)
        update.message.reply_text(f\"Generated key: `{key}` valid for {months} month(s) until {expires.isoformat()} (UTC)\", parse_mode=ParseMode.MARKDOWN)
    ...
    def keys_list_cmd(update: Update, context: CallbackContext):
        ...
        text = \"Keys:\\n\"
        for r in rows:
            text += f\"{r[0]} | months={r[1]} | created={r[2]} | expires={r[3]} | used_by={r[4]}\\n\"
        update.message.reply_text(text)
    ...
    def apply_pro_key(chat_id, user_id, key):
        ...
        cur.execute(\"UPDATE pro_keys SET used_by=?, expires_at=? WHERE key=?\", (user_id, row[3], key))
    ...
    def message_handler(update: Update, context: CallbackContext):
        ...
        # If nolinks enabled: detect URLs
        ...
        violated = False
        reason = None
        # check blacklist words first
        for b in bl:
            if b.lower() in text.lower():
                violated = True; reason = f'blacklist match {b}'; break
        if violated:
            try:
                msg.delete()
        ...
        if s['nolinks'] and urls:
            ...
            for u in urls:
                for w in wl:
                    if w.lower() in u.lower() or w.lower() in text.lower():
                        allowed = True; break
                if allowed: break
            if not allowed:
                try:
                    msg.delete(); logger.info('deleted message with link by %s', msg.from_user.id)
                except Exception as e:
                    logger.warning('delete failed: %s', e)
                return
        if s['nolinks'] and mentions:
            # if mention leads to disallowed username pattern, delete
            for m in mentions:
                # check if mention itself is whitelisted string
                if any(w.lower() in m.lower() for w in wl):
                    continue
                try:
                    msg.delete(); logger.info('deleted message with mention @%s by %s', m, msg.from_user.id)
                except Exception as e:
                    logger.warning('delete failed: %s', e)
                return
    ...
    def main():
        init_db()
        if not BOT_TOKEN:
            logger.error(\"BOT_TOKEN not set. Fill .env from .env.example\"); return
