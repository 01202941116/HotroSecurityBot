...
async def filter_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        return await update.message.reply_text("Cú pháp: <code>/filter_add @spam</code>", parse_mode="HTML")
    pattern = " ".join(context.args)
    db = SessionLocal(); f = Filter(chat_id=update.effective_chat.id, pattern=pattern); db.add(f); db.commit()
...
async def filter_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = SessionLocal(); items = db.query(Filter).filter_by(chat_id=update.effective_chat.id).all()
    if not items: return await update.message.reply_text("Danh sách filter trống.")
...
async def filter_del(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: return await update.message.reply_text("Cú pháp: /filter_del <id>")
    fid = int(context.args[0])
    db = SessionLocal(); it = db.query(Filter).filter_by(id=fid, chat_id=update.effective_chat.id).one_or_none()
    if not it: return await update.message.reply_text("Không tìm thấy ID.")
    db.delete(it); db.commit(); await update.message.reply_text(f"🗑️ Đã xoá filter #{fid}.")

async def toggle(update: Update, field:str, val:bool, label:str):
    db = SessionLocal(); s = db.query(Setting).filter_by(chat_id=update.effective_chat.id).one_or_none()
...
async def setflood(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = SessionLocal(); s = db.query(Setting).filter_by(chat_id=update.effective_chat.id).one_or_none()
    if not s: s = Setting(chat_id=update.effective_chat.id); db.add(s)
    s.flood_limit = n; db.commit(); await update.message.reply_text(f"✅ Flood limit = {n}")
...
async def guard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # anti forward
    if s.antiforward and (msg.forward_date or msg.forward_origin):
        try: await msg.delete()
        except Exception: pass
        return
    ...
    if len(bucket) > s.flood_limit and s.flood_mode == "mute":
        try:
            ...
            await context.bot.restrict_chat_member(chat_id, msg.from_user.id, ChatPermissions(can_send_messages=False), until_date=until)
        except Exception: pass
